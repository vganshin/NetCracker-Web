...
import { checkSubscriptionByTokenId } from 'utils';
...
export default function episodes(state = DEFAULT_STATE, action) {
  ...
  switch (type){
    case ADD_NEW_EPISODE:
      if (checkSubscriptionByTokenId(episode.token.id)) {
        return { ...state, generalEpisodes: [episode, ...state.generalEpisodes], favouriteEpisodes: [episode, ...state.favouriteEpisodes]};
      } else {
